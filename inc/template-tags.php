...
function wpfolio_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'wpfolio' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'wpfolio' ),
...
if ( ! function_exists( 'wpfolio_entry_footer' ) ) :
/**
...
function wpfolio_entry_footer() {
	...
	if ( 'post' === get_post_type() && ! in_category( 'project' ) ) {
		...
		if ( $categories_list && wpfolio_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'wpfolio' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'wpfolio' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'wpfolio' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
...
function wpfolio_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'wpfolio_categories' ) ) ) {
		...
		set_transient( 'wpfolio_categories', $all_the_cool_cats );
	}
...
function wpfolio_category_transient_flusher() {
	...
	// Like, beat it. Dig?
	delete_transient( 'wpfolio_categories' );
}
